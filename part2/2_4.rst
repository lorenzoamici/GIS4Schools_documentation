...
Now, click on **Vector → Geoprocessing Tools → Dissolve (1)**. Select as **Input layer**: *Municipalities*. Select **Dissolve field**: *Province* (after clicking on the ellipses symbol) **(3)**. Save the **Dissolved** layer, namely *Municipalities_on_lakes_diss.shp*, in the usual folder **(4)**. Finally, click **Run (5)**. :numref:`2.4.4.1` summarizes the procedure and shows the result.
